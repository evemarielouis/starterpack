...
class PacksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pack" do
    assert_difference("Pack.count") do
      post packs_url, params: { pack: { icon: @pack.icon, name: @pack.name, title: @pack.title } }
    end
  ...
  test "should update pack" do
    patch pack_url(@pack), params: { pack: { icon: @pack.icon, name: @pack.name, title: @pack.title } }
